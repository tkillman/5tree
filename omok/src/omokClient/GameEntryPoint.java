   ...
   public void start(Stage stage) throws Exception {
      ...
      root.getChildren().add(chatInput);
        ...
        // 게임시작 버튼을 만들어 준다.
         ...
         // 버튼 누른 이벤트 설정
         /*
          * startBtn.setOnAction((event) -> { if
          * (startBtn.getText().equals("게임 시작")) { startBtn.setText("게임 기권");
          * 
          * } });
          */
      ...
      // omokPan.setOnMouseClicked((event) -> {
      // if(!(event.getX() < 8 || event.getX() > 572 || event.getY() < 8 ||
      // event.getY() > 572)){
      // try {
      // String xyPoint =
      // "point:"+String.valueOf(point[0])+","+String.valueOf(point[1]);
      // byte[] bt = xyPoint.getBytes("UTF-8");
      // OutputStream sender = connSock.getOutputStream();
      // sender.write(bt);
      //
